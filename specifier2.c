...
bufferIndex++;
return (writeUnsignedNumber(0, bufferIndex, outputBuffer, flags, width, precision, sizeSpecifier));
}
...
outputBuffer[i--] = '0';
i++;
return (writeUnsignedNumber(0, i, outputBuffer, calcFlags, widthSpec, precisionSpec, sizeSpec));
}

/**
 * print_Hexadecimal_num - Outputs an unsigned number using hexadecimal notation
 ...
 */
int print_Hexadecimal_num(va_list types, char buffer[], int flags, int width, int precision, int size) {
return (printed_Hexadecimal(types, "0123456789abcdef", buffer, flags, 'x', width, precision, size));
}
...
i++;
return (writeUnsignedNumber(0, i, outputBuffer, flags, width, precision, size));
}
